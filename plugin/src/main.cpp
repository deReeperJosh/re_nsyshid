...
WUPSButtonCombo_Buttons DEFAULT_PRESS_DOWN_BUTTON_COMBO =
    WUPS_BUTTON_COMBO_BUTTON_L | WUPS_BUTTON_COMBO_BUTTON_R;
WUPSButtonCombo_Buttons DEFAULT_PRESS_HOLD_COMBO =
    WUPS_BUTTON_COMBO_BUTTON_L | WUPS_BUTTON_COMBO_BUTTON_R | WUPS_BUTTON_COMBO_BUTTON_DOWN;
...
{
  DEBUG_FUNCTION_LINE_INFO("New value in buttonComboItemChanged: %d for %s", newValue,
                           item->identifier);
}
...
WUPSConfigAPICallbackStatus ConfigMenuOpenedCallback(WUPSConfigCategoryHandle rootHandle)
{
  ...
  // The functions of the Config API come in two variants: One that throws an exception, and another
  // one which doesn't To use the Config API without exception see the example below this try/catch
  // block.
  try
  {
    ...
    root.add(WUPSConfigItemIntegerRange::Create(
        INTEGER_RANGE_EXAMPLE_CONFIG_ID, "Item for selecting an integer between 0 and 50",
        INTEGER_RANGE_DEFAULT_VALUE, sIntegerRangeValue, 0, 50, &integerRangeItemChanged));

    // To change a button combo, we can use the ButtonCombo ConfigItem
    root.add(WUPSConfigItemButtonCombo::Create(
        BUTTON_COMBO_PRESS_DOWN_CONFIG_ID, "Press Down button combo",
        DEFAULT_PRESS_DOWN_BUTTON_COMBO, sPressDownExampleHandle, buttonComboItemChanged));
    ...
    root.add(WUPSConfigItemMultipleValues::CreateFromValue(
        MULTIPLE_VALUES_EXAMPLE_CONFIG_ID, "Select an option!", MULTIPLE_VALUES_DEFAULT_VALUE,
        sExampleOptionValue, possibleValues, nullptr));
  ...
  // If we add a "WUPSConfigAPIStatus" reference to the API calls, the function won't throw an
  // exception. Instead, it will return std::optionals and write the result into the
  // WUPSConfigAPIStatus.
  ...
  auto boolItemOpt = WUPSConfigItemBoolean::Create(
      OTHER_EXAMPLE2_BOOL_CONFIG_ID, "Just another bool item", false, false, boolItemChanged, err);
  if (!boolItemOpt)
  {
  ...
  {
    DEBUG_FUNCTION_LINE_ERR("Failed to add bool item to category: %s",
...
INITIALIZE_PLUGIN()
{
  ...
  // The combo will be added on construction of that wrapper, and removed again in the destructor.
  // Use `std::move` to move it around. Like the C++ config api there are two versions of all
  // function, one that throws an exception on error and one that returns a std::optional but set an
  ...
  try
  {
    ...
    // This version will check for conflicts. It's useful to check for conflicts if you want to use
    // that button combo for a global unique thing that's always possible, like taking screenshots.
    WUPSButtonComboAPI::ButtonCombo buttonComboPressDown = WUPSButtonComboAPI::CreateComboPressDown(
        ...
        [](const WUPSButtonCombo_ControllerTypes triggeredBy, WUPSButtonCombo_ComboHandle, void*) {
          DEBUG_FUNCTION_LINE_INFO("Button combo has been pressed down by controller %s",
                                   WUPSButtonComboAPI::GetControllerTypeStr(triggeredBy));
        },
        nullptr, comboStatus);
    // On success, we can check if the combo is actually active by checking the combo status.
    // If there is already another combo that conflicts with us, the status will be set to
    // WUPS_BUTTON_COMBO_COMBO_STATUS_CONFLICT
    switch (comboStatus)
    {
    ...
    // BUT. We need to make sure to keep that button combo instance. Otherwise, the combo will be
    // removed. We save it in this list, which gets cleared in DEINITIALIZE_PLUGIN
    ...
    // E.g. when a new Aroma update is detected, the updater can be launched by holding the PLUS
    // button. This should always be possible.

    // If we don't want to check for conflicts, we need to create a "PressDownObserver"
    WUPSButtonComboAPI::ButtonCombo buttonComboPressDownObserver =
        WUPSButtonComboAPI::CreateComboPressDownObserver(
            "Example Plugin: Press Down observer test",
            DEFAULT_PRESS_DOWN_BUTTON_COMBO,  // L + R Even though this is same combo as in
                                              // buttonComboPressDown an observer will ignore
                                              // conflicts.
            [](const WUPSButtonCombo_ControllerTypes triggeredBy, WUPSButtonCombo_ComboHandle,
               void*) {
              DEBUG_FUNCTION_LINE_INFO(
                  "[OBSERVER] Button combo has been pressed down by controller %s",
                  WUPSButtonComboAPI::GetControllerTypeStr(triggeredBy));
            },
            nullptr,
            comboStatus);  // comboStatus will always be WUPS_BUTTON_COMBO_COMBO_STATUS_VALID for
                           // observers.
    ...
    // In case of a conflict, the button combo instance will be returned, but the combo status will
    // be WUPS_BUTTON_COMBO_COMBO_STATUS_CONFLICT

    // Let's create a button combo which will lead to a conflict. This time we want to check if a
    // combo has been hold for 500ms. Conflicts are checked across non-observer combo types.
    WUPSButtonComboAPI::ButtonCombo buttonComboHold = WUPSButtonComboAPI::CreateComboHold(
        "Example Plugin: Hold test",
        DEFAULT_PRESS_HOLD_COMBO,  // L+R+DPAD+DOWN. This combo includes the combo "L+R" of the
                                   // buttonComboPressDown, so this will lead to a conflict.
        500,                       // We need to hold that combo for 500ms
        [](const WUPSButtonCombo_ControllerTypes triggeredBy, WUPSButtonCombo_ComboHandle, void*) {
          DEBUG_FUNCTION_LINE_INFO("Button combo has been hold for 500ms by controller %s",
                                   WUPSButtonComboAPI::GetControllerTypeStr(triggeredBy));
        },
        nullptr,
        comboStatus);  // comboStatus will always be WUPS_BUTTON_COMBO_COMBO_STATUS_VALID for
    ...
    case WUPS_BUTTON_COMBO_COMBO_STATUS_CONFLICT:
      DEBUG_FUNCTION_LINE_INFO(
          "Conflict detected for button combo");  // <-- this is expected to happen
      break;
    default:
      DEBUG_FUNCTION_LINE_ERR("Invalid combo status");
      break;
    ...
    // Once combo is in the "WUPS_BUTTON_COMBO_COMBO_STATUS_CONFLICT" state it can only be valid
    // again, if the combo or the controllerMask changes. Other combo won't ever affect this state
    // of this combo We can easily update the button combo
    if (const auto res = buttonComboHold.UpdateButtonCombo(WUPS_BUTTON_COMBO_BUTTON_ZR |
                                                               WUPS_BUTTON_COMBO_BUTTON_R |
                                                               WUPS_BUTTON_COMBO_BUTTON_DOWN,
                                                           comboStatus);
    ...
    else
    {
      ...
      case WUPS_BUTTON_COMBO_COMBO_STATUS_VALID:
        DEBUG_FUNCTION_LINE_INFO(
            "Button combo is valid and active");  // <-- this is expected to happen
        break;
      case WUPS_BUTTON_COMBO_COMBO_STATUS_CONFLICT:
        DEBUG_FUNCTION_LINE_INFO("Conflict detected for button combo");
        break;
    ...
    // If we want to create combo for only a specific controller (or specific controllers), we have
    // to use the "Ex" functions.

    // If we don't want to check for conflicts, we need to create a "PressDownObserver", but this
    // time we have to provide a controllerMask and if it's a observer.
    WUPSButtonComboAPI::ButtonCombo buttonComboPressDownExObserver =
        WUPSButtonComboAPI::CreateComboPressDownEx(
            "Example Plugin: Press Down observer test",
            WUPS_BUTTON_COMBO_CONTROLLER_WPAD_0,  // Define which controllers should be checked.
                                                  // Could be something
            ...
            [](const WUPSButtonCombo_ControllerTypes triggeredBy, WUPSButtonCombo_ComboHandle,
               void*) {
              DEBUG_FUNCTION_LINE_INFO(
                  "[OBSERVER WPAD_0] Button combo has been pressed down by controller %s",
                  WUPSButtonComboAPI::GetControllerTypeStr(triggeredBy));
            },
            nullptr,
  ...
  // We add a "WUPSButtonCombo_Error" parameter at the end, the function will not throw any
  // exception, but return a std::optional instead. Create an observer, because we don't care for
  // conflicts.
  std::optional<WUPSButtonComboAPI::ButtonCombo> buttonComboPressDownObserverOpt =
      WUPSButtonComboAPI::CreateComboPressDownObserver(
          ...
          [](const WUPSButtonCombo_ControllerTypes triggeredBy, WUPSButtonCombo_ComboHandle,
             void*) {
            DEBUG_FUNCTION_LINE_INFO(
                "[OBSERVER] Other button combo has been pressed down by controller %s",
                WUPSButtonComboAPI::GetControllerTypeStr(triggeredBy));
          },
...
/**
  ...
  Currently it's only possible to override functions that are loaded from .rpl files of OSv10
(00050010-1000400A).
...
**/
DECL_FUNCTION(int, FSOpenFile, FSClient* pClient, FSCmdBlock* pCmd, const char* path,
...
/**
This tells the loader which functions from which library (.rpl) should be replaced with which
function from this file. The list of possible libraries can be found in the wiki. (In general it's
WUPS_LOADER_LIBRARY_ + the name of the RPL in caps lock)

WUPS_MUST_REPLACE(FUNCTION_NAME_IN_THIS_FILE,   NAME_OF_LIB_WHICH_CONTAINS_THIS_FUNCTION,
NAME_OF_FUNCTION_TO_OVERRIDE)
